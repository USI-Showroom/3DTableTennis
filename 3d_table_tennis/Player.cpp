...
void PingPongPlayer::setElbow(const double x,const double y,const double z,const bool left)
{
    Point3d tmp(x,y,z);
    if (left)
        leftElbow = tmp;
...
Point3d PingPongPlayer::getLeftElbow()
{
    return leftElbow;
}

Point3d PingPongPlayer::getRightElbow()
{
    return rightElbow;
}
...
Point3d PingPongPlayer::getPlayingElbow()
{
    return isLefty ? leftElbow : rightElbow;
}
