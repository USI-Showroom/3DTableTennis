...
Point3d Paddle::getElbowPos()
{
	...
	Point3d elbowPosition = elbowPos.front();
	double px = elbowPosition.x();
	double py = elbowPosition.y() + CENTER_HEIGHT;
	double pz = elbowPosition.z() - CENTER_DISTANCE;
	return Point3d(px, py, pz);
}
...
void Paddle::updateElbowPos(const Point3d newElbowPos)
{
	elbowPos.push_front(newElbowPos);
	
	if(elbowPos.size() >= list_size)
	{
		elbowPos.pop_back();
	}
}
